...
class Evaluator:
    ...
    def _batch(self, model, prefix, batch, device, start=0, end=None):
        ...
        output = model.forward_qa_with_prefix(prefix, qa_indices, embed(ids["question_ids"][start:end]), ids["question_mask"][start:end], embed(ids["answer_ids"][start:end]), ids["answer_mask"][start:end], ids["labels"][start:end])
    ...
    def teacher_forced(self, model, loader, device):
        model.eval(); qa_sum = reconstruction_sum = 0.0; em = f1 = rouge = bleu = 0.0; samples = 0; qa_weight = context_weight = 0
        ...
        for batch in tqdm(loader, total=len(loader), desc="Validation (teacher-forced)", unit="batch", disable=not enabled):
            ...
            reconstruction = reconstruction_loss(prefix.reconstruction, prefix.context_target, prefix.context_mask, self.cfg.memory.reconstruction_cosine_weight, self.cfg.memory.reconstruction_loss)
            contexts = ids["context_ids"].size(0); reconstruction_sum += float(reconstruction) * contexts; context_weight += contexts
            for start in self._chunks(batch):
                end = min(start + self.cfg.evaluation.qa_batch_size, batch["question_ids"].size(0)); output, qa, ids, _ = self._batch(model, prefix, batch, device, start, end); count = end - start
                qa_sum += float(qa) * count; qa_weight += count
        ...
        model.train(); qa_value = qa_sum / max(1, qa_weight); reconstruction_value = reconstruction_sum / max(1, context_weight)
        total_value = self.cfg.memory.qa_weight * qa_value + self.cfg.memory.reconstruction_weight * reconstruction_value
        return {"qa_loss": qa_value, "ppl": math.exp(min(qa_value, 20)), "reconstruction_loss": reconstruction_value, "loss": total_value, "em": em / max(1, samples), "f1": f1 / max(1, samples), "rouge": rouge / max(1, samples), "bleu": bleu / max(1, samples)}
    ...
    def autoregressive(self, model, loader, device):
        ...
        for batch in tqdm(loader, total=len(loader), desc="Validation (autoregressive)", unit="batch", disable=not enabled):
            ids = {key: value.to(device) for key, value in batch.items() if key != "records"}; q = ids["qa_context_indices"]; embed = model.qwen.get_input_embeddings(); prefix = model.encode_context_prefix(embed(ids["context_ids"]), ids["context_mask"])
            for start in self._chunks(batch):
                end = min(start + self.cfg.evaluation.qa_batch_size, q.numel()); indices = q[start:end]
                generated = model.generate_answer_with_prefix(prefix, indices, ids["question_ids"][start:end], ids["question_mask"][start:end], self.tokenizer, self.cfg.evaluation.max_new_tokens)
                for i, record in enumerate(batch["records"][start:end]):
