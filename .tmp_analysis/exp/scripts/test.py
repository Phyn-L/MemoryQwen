...
def main():
    ...
    CheckpointManager(cfg.checkpoint.output_dir).load(checkpoint_path, model)
    ...
    collate = lambda rows: collate_fn(rows, tokenizer, cfg.data.max_context_tokens, cfg.data.max_question_tokens, cfg.data.max_answer_tokens, cfg.data.append_eos, cfg.data.use_chat_template, cfg.data.chat_template_enable_thinking, cfg.data.qa_per_context, False, cfg.data.question_padding_side, cfg.data.eos_mode)
    loader = DataLoader(
