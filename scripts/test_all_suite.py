        ...
        for line in path.open():
            for qa in json.loads(line).get('qa_pairs', []):
                if version and qa.get('source_dataset') != version:
                    continue
                total += 1
                answered += bool(qa.get('answers') and any(str(x).strip() for x in qa['answers']))
        if not answered:
            manifest.append(dict(dataset=name, split=split, total=total, status='unscorable:no_reference_answers'))
            (out / 'manifest.json').write_text(json.dumps(manifest, indent=2)+'\n')
            print(f'{name}: {total} rows, no reference answers; scoring skipped', flush=True)
            continue
        common = ['--machine', args.machine, '--datasets', dataset, '--split', split]
        if version:
            common += ['--source-version', version]
        commands = [('memory', [sys.executable, '-m', 'utils.launcher', 'test',
                    '--ckpt', args.ckpt, '--no-filter-long-context', '--bs', str(args.memory_bs), *common])]
