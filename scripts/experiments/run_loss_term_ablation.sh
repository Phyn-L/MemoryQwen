...
# Independent loss-term ablations. Every run starts from the same pretrained
# backbone and differs only in the enabled auxiliary loss terms.
BASE_CONFIG=${BASE_CONFIG:-configs/train_baseline.yaml}
OUTPUT_ROOT=${OUTPUT_ROOT:-outputs/ab_loss_terms}
MACHINE=${MACHINE:-4090}
NUM_PROCESSES=${NUM_PROCESSES:-}
DRY_RUN=0

RUNS=(00_full 01_no_distill 02_no_causal_recon 03_no_token_recon 04_qa_only)
...
if [[ -n "$NUM_PROCESSES" ]]; then
  [[ "$NUM_PROCESSES" =~ ^[1-9][0-9]*$ ]] || {
    echo "--num-processes must be a positive integer" >&2
    exit 2
  }
...
# Keep the experiment controlled: only the loss weights differ across runs.
runs = {
    "00_full": {},
    "01_no_distill": {"distill_weight": 0.0},
    "02_no_causal_recon": {"causal_recon_weight": 0.0},
    "03_no_token_recon": {"token_recon_weight": 0.0},
    "04_qa_only": {
        "token_recon_weight": 0.0,
        "causal_recon_weight": 0.0,
        "distill_weight": 0.0,
    },
}
...
for name, overrides in runs.items():
    ...
    logging["wandb_run_name"] = f"{out_root.name}_{name}"
