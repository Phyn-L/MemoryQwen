...
| E8 | `src/losses.py:44-50` | `memory_contrastive_loss` 的 positive 项 `(z*z).sum(-1)/T ≡ 1/T` 是常数，实际只剩一个阈值式排斥项 | 语义与注释不符（weight=0 未启用）；该未启用目标随后已删除，见 `docs/guides/READER_OPTIONS.md` |
...
12. ~~修 `memory_contrastive_loss` 的常数 positive 项，或换成 in-batch 对比（同 context 的
    ...
    若日后仍想做"同 context 为正、跨 context 为负"的对比，请按 `docs/guides/READER_OPTIONS.md`
